...
int main(int argc, LPCTSTR* argv)
{
	...
	// Binary write
	std::string filename = "/home/hp/Desktop/cameras_krc.bin";
	FILE* fwp = fopen(filename.c_str(), "wb");
	
	// Export cameras
	std::ofstream sfwrite("/home/hp/Desktop/camera.txt", std::ios::out);
	std::ofstream sfwrite_krc("/home/hp/Desktop/KRC.txt", std::ios::out);

	int num_images = scene.images.size();
	std::cout << "Platform  size is \n" << scene.platforms.size() << "\n";
	std::cout << "Platform camera size is \n" << scene.platforms[0].cameras.size() << "\n";
	std::cout << "Platform camera is \n" << scene.platforms[0].cameras[0].K << "\n";
	
	fwrite(&(num_images), sizeof(int), 1, fwp);
	for (int i = 0; i < num_images; i++){
		std::cout << scene.images[i].name << "\n";
		std::cout << scene.images[i].width << "\n";
		std::cout << scene.images[i].height << "\n";
		
		// auto name = scene.images[i].name.c_str();
		// int name_len = sizeof(name);
		// fwrite(&(name_len), sizeof(int), 1, fwp);
		// fwrite(name, sizeof(name[0]), name_len, fwp);

		std::vector<char> name(scene.images[i].name.begin(), scene.images[i].name.end());
		int name_len = name.size();
		fwrite(&(name_len), sizeof(int), 1, fwp);
		fwrite(&name[0], sizeof(name[0]), name_len, fwp);

		fwrite(&(scene.images[i].camera.K), sizeof(scene.images[i].camera.K), 1, fwp);
		fwrite(&(scene.images[i].camera.R), sizeof(scene.images[i].camera.R), 1, fwp);
		fwrite(&(scene.images[i].camera.C), sizeof(scene.images[i].camera.C), 1, fwp);
		
		sfwrite_krc << "ImagePath\n" << scene.images[i].name << "\n";
		sfwrite_krc << "K \n" << scene.images[i].camera.K << "\n";
		sfwrite_krc << "R \n" << scene.images[i].camera.R << "\n";
		sfwrite_krc << "C \n" << scene.images[i].camera.C << "\n\n";

		std::cout << i << "Image scale is " << scene.images[i].scale << "\n\n";
		sfwrite << "ImagePath\n" << scene.images[i].name << "\n";
		sfwrite << "CameraPose\n" << scene.images[i].camera.P << "\n\n";
	}
	sfwrite.close();
	sfwrite_krc.close();
	fclose(fwp);
